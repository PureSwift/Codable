...
*/
  ...
  func testDecodingConcreteTypeParameter() {
    ...
    XCTAssert(type(of: decoded) == Employee.self, "Expected decoded value to be of type Employee; got \(type(of: decoded)) instead.")
  }*/
...
private enum OptionalTopLevelWrapperCodingKeys : String, CodingKey {
    case value
}
...
fileprivate struct OptionalTopLevelWrapper<T> : Codable, Equatable where T : Codable, T : Equatable {
  ...
  init(from decoder: Decoder) throws {
    let container = try decoder.container(keyedBy: OptionalTopLevelWrapperCodingKeys.self)
    value = try container.decode(Optional<T>.self, forKey: .value)
  }

  func encode(to encoder: Encoder) throws {
    var container = encoder.container(keyedBy: OptionalTopLevelWrapperCodingKeys.self)
...
func expectUnreachable(_ error: String = "", file: StaticString = #file, line: UInt = #line) {
